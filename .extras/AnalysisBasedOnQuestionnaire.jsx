...
const Analysis = () => {
  ...
  const [selectedQuestionnaireName, setSelectedQuestionnaireName] =
    useState("");
  ...
  const getValForSeries = () => {
    const series = ["Not Answered", "Incorrect Answer", "Correct Answer"].map(
      (name) => ({ name, data: Array(attendeeNameArr.length).fill(0) })
    );
  ...
  const options = {
    ...
    title: {
      text: selectedQuestionnaireName, // `${selectedAttendeeName} - ${selectedQuestionnaireName}`,
      // align: "left",
    },
    // subtitle: {
    ...
    xAxis: {
      categories: attendeeNameArr, // !Project Title here
      title: {
  ...
  // console.log("in analytics global==",selectedQuestionnaireId,selectedQuestionnaireName);
  return (
    <>
